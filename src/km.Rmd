...
ggsurv <- ggsurvplot_combine(fitall,
  data = wdata, censor = FALSE, surv.scale = "percent", xscale = 365.25,
  break.x.by = c(365.25 * 2), xlim = c(0, 365.25 * 12.2),
  ylab = "Survival/HF hospitalisation survival free (%)", xlab = "Years",
  risk.table = FALSE,
...
ggsurv$plot <- ggsurv$plot +
  ggplot2::annotate("text",
    x = 970, y = 0.22, # x and y coordinates of the text
    label = paste0("Log-rank AC mortality p", p1), size = 4.5
  ) +
  ggplot2::annotate("text",
    x = 1125, y = 0.17, # x and y coordinates of the text
...
ggsurv <- ggsurvplot_combine(fitall,
  data = wdata %>% filter(location == "in-patient"), censor = FALSE, surv.scale = "percent", xscale = 365.25,
  break.x.by = c(365.25 * 2), xlim = c(0, 365.25 * 12.2),
  ylab = "Survival/HF hospitalisation survival free (%)", xlab = "Years",
  risk.table = FALSE,
  conf.int = FALSE, palette = kicols,
...
ggsurv <- ggsurvplot_combine(fitall,
  data = wdata %>% filter(location == "out-patient"), censor = FALSE, surv.scale = "percent", xscale = 365.25,
  break.x.by = c(365.25 * 2), xlim = c(0, 365.25 * 12.2),
  ylab = "Survival/HF hospitalisation survival free (%)", xlab = "Years",
  risk.table = FALSE,
  conf.int = FALSE, palette = kicols,
